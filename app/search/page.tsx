...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const matchedRepos = needle
    ? stackRows.filter((row) => `${row.owner}/${row.name} ${row.description ?? ''}`.toLowerCase().includes(needle))
    : [];
  ...
  const releaseRows =
    needle && repoIds.length
      ? await db
          ...
          .where(and(inArray(releases.repoId, repoIds), isNotNull(releases.publishedAt)))
  ...
  const nothing = needle && matchedRepos.length === 0 && matchedReleases.length === 0 && githubResults.length === 0;
    ...
    <div className="flex w-full flex-col gap-6">
      ...
      {nothing && (
        <Empty className="rounded-lg ring-1 ring-foreground/10">
          <EmptyHeader>
            ...
            <EmptyDescription>Try a different term, or search GitHub directly.</EmptyDescription>
          </EmptyHeader>
        </Empty>
      )}
        ...
        <section className="flex flex-col gap-2">
          ...
          <ul className="flex flex-col divide-y divide-border overflow-hidden rounded-lg ring-1 ring-foreground/10">
            {matchedReleases.map((row) => (
              <li key={row.releaseId} className="flex flex-col gap-1 px-3 py-2.5 transition-colors hover:bg-muted">
                <div className="flex flex-wrap items-baseline gap-2">
                  ...
                  <span className="ml-auto text-muted-foreground text-xs">{formatDate(row.publishedAt)}</span>
                </div>
                <span className="text-muted-foreground text-sm">{row.summary.headline}</span>
              </li>
            ))}
          </ul>
        </section>
      ...
      {githubResults.length > 0 && (
        <section className="flex flex-col gap-2">
          ...
          <GitHubReposTable repos={githubResults} repoIdByFullName={repoIdByFullName} emptyMessage="No matches." />
        </section>
      )}
    </div>
  );
