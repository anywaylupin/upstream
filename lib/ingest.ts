...
async function fetchReleases(
  ...
  if (!res.ok) {
    throw new Error(
      `GitHub API error for ${owner}/${name}: ${res.status} ${res.statusText}`,
    );
  }
...
export async function ingestRepo(repo: {
  id: number;
  owner: string;
  name: string;
}) {
  ...
  const meta = await getRepoMeta(repo.owner, repo.name, GITHUB_TOKEN).catch(
    (err: unknown) => {
      console.error(
        `metadata refresh failed for ${repo.owner}/${repo.name}:`,
        err,
      );
      return null;
    },
